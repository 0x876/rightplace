...
class Ethers extends ts_generator_1.TsGeneratorPlugin {
    ...
    transformFile(file) {
        const fileExt = shared_1.getFileExtension(file.path);
    ...
    transformBinFile(file) {
        ...
        const bytecode = abiParser_1.extractBytecode(file.contents);
        if (!bytecode) {
            return;
        }
    ...
    transformAbiOrFullJsonFile(file) {
        ...
        const bytecode = abiParser_1.extractBytecode(file.contents) || this.bytecodeCache[name];
        if (bytecode) {
            return [
